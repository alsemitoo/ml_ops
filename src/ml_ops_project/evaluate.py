...
def get_test_dataset(data_path: Path, tokenizer: LaTeXTokenizer):
    ...
    base_dataset = MyDataset(
        data_path=data_path, tokenizer=tokenizer, transform=transform 
    )
...
def test(cfg: DictConfig):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=test_cfg.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=4)
    
    model = Im2LatexModel(vocab_size=vocab_size, d_model=model_cfg.d_model, nhead=model_cfg.nhead, num_decoder_layers=model_cfg.num_decoder_layers)
