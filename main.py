...
def beam_search_prediction(model, image, tokenizer, beam_width=3, max_len=150):
    ...
    if isinstance(best_seq, int): best_seq = [best_seq]
    
    # Clean SOS/EOS
    if len(best_seq) > 0 and best_seq[0] == sos_id: best_seq = best_seq[1:]
    if len(best_seq) > 0 and best_seq[-1] == eos_id: best_seq = best_seq[:-1]
